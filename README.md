...
**基于 [sansan0/TrendRadar](https://github.com/sansan0/TrendRadar) 的二次开发扩展**  
在原项目热榜聚合能力之上,新增 **154 个 MCP 工具**、本地语义搜索、定时任务编排、飞书反向通道、Obsidian 导出、跨平台叙事追踪等能力。

遵循原项目 **GPL-3.0** 协议。完整归属与差异见 [NOTICE.md](NOTICE.md)。

---

<a href="https://github.com/sansan0/TrendRadar" title="Upstream: sansan0/TrendRadar">
  <img src="/_image/banner.webp" alt="Argus Banner (Upstream: TrendRadar)" width="80%">
</a>

热点聚合 + AI-Agent 个人情报中枢
...
[![GitHub Pages](https://img.shields.io/badge/GitHub_Pages-部署-4285F4?style=flat-square&logo=github&logoColor=white)](https://sansan0.github.io/Argus)
[![Docker](https://img.shields.io/badge/Docker-部署-2496ED?style=flat-square&logo=docker&logoColor=white)](https://hub.docker.com/r/wantcat/argus)
...
>
> 无论名字是否上榜，你们的每一份支持都是 Argus 能够走到今天的基石。🙏
...
|           *W          |  6  | 2025.11.13  |      | 
|           *凯          |  1  | 2025.11.13  |      | 
|           对*.          |  1  | 2025.11.13  |    Thanks for your Argus  | 
|           s*y          |  1  | 2025.11.13  |      | 
|           **翔          |  10  | 2025.11.13  |   好项目，相见恨晚，感谢开源！     | 
|           *韦          |  9.9  | 2025.11.13  |   Argus超赞，请老师喝咖啡~     | 
...
> 若 Argus 曾为你捕捉价值，不妨为它注入动力，助其持续进化
>
...
- **代码复用优化**：批次处理函数直接复用 argus 核心模块，不重复造轮子
...
- **新增 check_version 工具**：支持同时检查 Argus 和 MCP Server 版本更新
...
- **核心代码模块化**：将主程序逻辑拆分为 argus 包的多个模块，显著提升代码可维护性。
...
- 新增独立的 MCP 服务镜像 `wantcat/argus-mcp`
...
- 通过 `manage.py` 命令控制启动/停止：`docker exec -it argus python manage.py start_webserver`
...
- 正式发布官方 Docker 镜像 wantcat/argus，支持多架构
...
👉 **在线体验**：[https://sansan0.github.io/Argus/](https://sansan0.github.io/Argus/)
...
>
> **💡 提示**：启用 GitHub Pages 可获得在线访问地址，进入仓库 Settings → Pages 即可开启。[效果预览](https://sansan0.github.io/Argus/)
>
...
### 2️⃣ 第二步：设置 GitHub Secrets
   ...
   **配置 Argus**：
   ...
   NTFY_TOPIC: argus-alerts  # 自托管可用简单名称
   ...
   **常见问题：**
   ...
   如果你选择随机的、足够长的名称（如 `argus-zs-8492-news`），暴力破解几乎不可能：
   ...
   **设置步骤：**
   ...
   3. **填写 App 信息**：
      - **App Name**：填写应用名称（如 `Argus` 或 `热点新闻监控`）
   ...
   1. **查看生成的 URL**：
      ...
      - 格式如:`https://hooks.slack.com/services/<TEAM_ID>/<BOT_ID>/<TOKEN>`
...
### 3️⃣ 第三步：手动测试新闻推送
   ...
   **如何找到你的 Actions 页面**：
   ...
   - **方法二**：直接访问 `https://github.com/你的用户名/Argus/actions`

   **示例对比**：
   - ❌ 作者的项目：`https://github.com/sansan0/TrendRadar/actions`
   - ✅ 你的项目：`https://github.com/你的用户名/Argus/actions`
...
### 5️⃣ 第五步：远程云存储 & 签到配置
   ...
   **GitHub Secret 配置（需添加 4 项）：**
   ...
   | Name（名称） | Secret（值）说明 |
   |-------------|-----------------|
   | `S3_BUCKET_NAME` | 存储桶名称（如 `argus-data`） |
   ...
   **详细操作步骤（获取凭据）：**
   ...
   3. **创建 API 令牌**：
      ...
      - **⚠️ 关键设置**：
        ...
        - **权限**：选择 `管理员读和写` 。
        - **指定存储桶**：为了安全，建议选择 `仅适用于指定存储桶` 并选中你的桶（如 `argus-data`）。
...
Argus 提供两个独立的 Docker 镜像，可根据需求选择部署：

| 镜像名称 | 用途 | 说明 |
|---------|------|------|
| `wantcat/argus` | 新闻推送服务 | 定时抓取新闻、推送通知（必选） |
| `wantcat/argus-mcp` | AI 分析服务 | MCP 协议支持、AI 对话分析（可选） |

> 💡 **建议**：
> - 只需要推送功能：仅部署 `wantcat/argus` 镜像
> - 需要 AI 分析功能：同时部署两个镜像
...
3. **启动服务**:
   ...
   **选项 C：仅启动 MCP AI 分析服务**
   ```bash
   # 只启动 argus-mcp（提供 AI 分析接口）
   docker compose pull argus-mcp
   docker compose up -d argus-mcp
   ```

   > 💡 **提示**：
   > - 大多数用户只需启动 `argus` 即可实现新闻推送功能
   > - 只有需要使用 ChatGPT/Gemini 进行 AI 对话分析时，才需启动 `argus-mcp`
   > - 两个服务相互独立，可根据需求灵活组合
...
4. **查看运行状态**:
   ...
   # 停止特定服务
   docker compose stop argus      # 停止推送服务
   docker compose stop argus-mcp  # 停止 MCP 服务
   ```
...
| 镜像名称 | 用途 | 说明 |
|---------|------|------|
| `wantcat/argus` | 新闻推送服务 | 定时抓取新闻、推送通知 |
| `wantcat/argus-mcp` | MCP 服务 | AI 分析功能（可选） |
...
docker exec -it argus python manage.py start_webserver   # 启动 Web 服务器
docker exec -it argus python manage.py stop_webserver    # 停止 Web 服务器
docker exec -it argus python manage.py webserver_status  # 查看 Web 服务器状态
...
> - 手动停止：`docker exec -it argus python manage.py stop_webserver`
> - 手动启动：`docker exec -it argus python manage.py start_webserver`
...
Argus 生成的当日汇总 HTML 报告会同时保存到两个位置：
...
Argus v3.0.0 新增了基于 **MCP (Model Context Protocol)** 的 AI 分析功能，让你可以通过自然语言与新闻数据对话，进行深度分析。
...
Argus MCP 服务支持标准的 Model Context Protocol (MCP) 协议，可以接入各种支持 MCP 的 AI 客户端进行智能分析。

### 支持的客户端

**注意事项**：
- 将 `/path/to/Argus` 替换为你的项目实际路径
- Windows 路径使用双反斜杠：`C:\\Users\\YourName\\Argus`
- 保存后记得重启
...
任何支持 Model Context Protocol 的客户端都可以连接 Argus：
