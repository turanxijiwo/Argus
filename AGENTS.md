...
API and MCP surface:
- `argus_server/server.py` registers 173 FastMCP tools and 8 MCP resources.
...
- AI/search: `AI_API_KEY`, `AI_MODEL`, `AI_API_BASE`, `AI_TEMPERATURE`, `AI_MAX_TOKENS`, `AI_TIMEOUT`, `TAVILY_API_KEY`, `EXA_API_KEY`, `PERPLEXITY_API_KEY`, `BRAVE_API_KEY`, `ARGUS_CODEX_MODEL`, `GITHUB_TOKEN`, `SEMANTIC_SCHOLAR_API_KEY`, `OPENALEX_API_KEY`, `RELIEFWEB_APPNAME`.
...
- External CLI: `ARGUS_VAULT`, `TWITTER_AUTH_TOKEN`, `TWITTER_CT0`, `TWITTER_BROWSER`, `TWITTER_CHROME_PROFILE`, `TWITTER_PROXY`, `TG_API_ID`, `TG_API_HASH`, `TG_SESSION_NAME`, `DATA_DIR`, `DB_PATH`, and `PATH` entries for optional CLI tools.
