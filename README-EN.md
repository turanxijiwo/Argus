<div align="center" id="argus">

<a href="https://github.com/sansan0/TrendRadar" title="Argus">
  <img src="/_image/banner.webp" alt="Argus Banner" width="80%">
</a>

Deploy in <strong>30 seconds</strong> — Say goodbye to endless scrolling, only see the news you truly care about

<a href="https://trendshift.io/repositories/14726" target="_blank"><img src="https://trendshift.io/api/badge/repositories/14726" alt="sansan0%2FArgus | Trendshift" style="width: 250px; height: 55px;" width="250" height="55"/></a>
...
[![GitHub Pages](https://img.shields.io/badge/GitHub_Pages-Deployment-4285F4?style=flat-square&logo=github&logoColor=white)](https://sansan0.github.io/Argus)
[![Docker](https://img.shields.io/badge/Docker-Deployment-2496ED?style=flat-square&logo=docker&logoColor=white)](https://hub.docker.com/r/wantcat/argus)
...
>
> Whether your name is on the list or not, your every bit of support is the cornerstone that allows Argus to be where it is today. 🙏
...
| *W | 6 | 2025.11.13 | |
| *凯 | 1 | 2025.11.13 | |
| 对*. | 1 | 2025.11.13 | Thanks for Argus |
| s*y | 1 | 2025.11.13 | |
| **翔 | 10 | 2025.11.13 | Wish I found it earlier |
| *韦 | 9.9 | 2025.11.13 | Argus is awesome |
...
### ❤️ Find it useful? Support Argus

> If Argus has captured value for you, give it some fuel to keep evolving
>
...
- **Code reuse**: Batch utilities now imported from argus core instead of duplicated
...
- Added visual configuration editor for Argus
...
- **New check_version Tool**: Check Argus and MCP Server version updates simultaneously
...
- Added independent MCP service image `wantcat/argus-mcp`
...
- Control via `manage.py` commands: `docker exec -it argus python manage.py start_webserver`
...
- Officially released official Docker image wantcat/argus, supports multi-architecture
...
👉 **Try it online**: [https://sansan0.github.io/Argus/](https://sansan0.github.io/Argus/)
...
No longer controlled by platform algorithms, Argus reorganizes all trending searches
...
>
> **💡 Tip**: Enable GitHub Pages for an online URL. Go to Settings → Pages to enable. [Preview Effect](https://sansan0.github.io/Argus/)
>
...
7. Message title fill "Argus Trending Monitor"
...
8. Message title fill "Argus Trending Monitor"
...
2. **Subscribe to Topic** (choose a hard-to-guess name):
   ```
   Suggested format: argus-{your initials}-{random numbers}

   Cannot use Chinese

   ✅ Good example: argus-zs-8492
...
NTFY_TOPIC: argus-alerts  # Self-hosting can use simple name
...
If you choose a random, sufficiently long name (like `argus-zs-8492-news`), brute force is nearly impossible:
...
3. **Fill in App Information**:
   - **App Name**: Enter app name (e.g., `Argus` or `Hot News Monitor`)
...
### 3️⃣ Step 3: Manual Test News Push
   ...
   **How to find your Actions page**:
   ...
   - **Method 2**: Direct access `https://github.com/YourUsername/Argus/actions`

   **Example comparison**:
   - ❌ Author's project: `https://github.com/sansan0/TrendRadar/actions`
   - ✅ Your project: `https://github.com/YourUsername/Argus/actions`
...
### 5️⃣ Step 5: GitHub Actions Check-In & Remote Cloud Storage
   ...
   **Required Configuration (4 items):**
   ...
   |------|----------------------------|
   | `S3_BUCKET_NAME` | Bucket name (e.g., `argus-data`) |
   ...
   **How to Get Credentials (Using Cloudflare R2 as Example):**
   ...
   2. Select `R2` in left menu → Click `Create Bucket` → Enter name (e.g., `argus-data`)
...
### 7️⃣ Step 7: 🎉 Deployment Success!
   ...
   Congratulations! Now you can start enjoying the efficient information flow brought by Argus.
...
### 8️⃣ Step 8: Advanced: Choose Your AI Assistant
   ...
   Argus provides two ways to use AI to meet different needs:
...
Argus provides two independent Docker images, deploy according to your needs:

| Image Name | Purpose | Description |
|---------|------|------|
| `wantcat/argus` | News Push Service | Scheduled news crawling, push notifications (Required) |
| `wantcat/argus-mcp` | AI Analysis Service | MCP protocol support, AI dialogue analysis (Optional) |

> 💡 **Recommendations**:
> - Only need push functionality: Deploy `wantcat/argus` image only
...
3. **Start Service**:
   ...
   **Option A: Start All Services (Push + AI Analysis)**
   ...
   # Start all services (argus + argus-mcp)
   docker compose up -d
   ```

   **Option B: Start News Push Service Only**
   ```bash
   # Start argus only (scheduled crawling and push)
   ...
   **Option C: Start MCP AI Analysis Service Only**
   ```bash
   # Start argus-mcp only (AI analysis interface)
   docker compose pull argus-mcp
   docker compose up -d argus-mcp
   ```

   > 💡 **Tips**:
   > - Most users only need to start `argus` for news push functionality
   > - Only need to start `argus-mcp` when using ChatGPT/Gemini for AI dialogue analysis
...
4. **Check Running Status**:
   ...
   docker compose stop argus      # Stop push service
   docker compose stop argus-mcp  # Stop MCP service
   ```
...
docker compose build argus-mcp
docker compose up -d argus-mcp
```
...
docker pull wantcat/argus:latest
docker pull wantcat/argus-mcp:latest
docker compose down
docker compose up -d
...
| Image Name | Purpose | Description |
|---------|------|---------|
| `wantcat/argus` | News Push Service | Scheduled news crawling, push notifications |
| `wantcat/argus-mcp` | MCP Service | AI analysis features (optional) |

#### Service Management Commands

```bash
# View running status
docker exec -it argus python manage.py status

# Manually execute crawler once
docker exec -it argus python manage.py run

# View real-time logs
docker exec -it argus python manage.py logs

# Display current config
docker exec -it argus python manage.py config

# Display output files
docker exec -it argus python manage.py files

# Web server management (for browser access to generated reports)
docker exec -it argus python manage.py start_webserver   # Start web server
docker exec -it argus python manage.py stop_webserver    # Stop web server
docker exec -it argus python manage.py webserver_status  # Check web server status

# View help info
docker exec -it argus python manage.py help
...
> - Stop manually: `docker exec -it argus python manage.py stop_webserver`
> - Start manually: `docker exec -it argus python manage.py start_webserver`
...
Argus generates daily summary HTML reports to two locations simultaneously:
...
# 1. Start web server
docker exec -it argus python manage.py start_webserver
# 2. Access in browser
...
docker inspect argus

# View container logs
docker logs --tail 100 argus

# Enter container for debugging
docker exec -it argus /bin/bash

# Verify config files
docker exec -it argus ls -la /app/config/
```
...
```bash
# Linux/Mac
docker run -d --name argus-mcp \
  -p 127.0.0.1:3333:3333 \
  -v $(pwd)/config:/app/config:ro \
  -v $(pwd)/output:/app/output:ro \
  -e TZ=Asia/Shanghai \
  wantcat/argus-mcp:latest
...
Argus v3.0.0 added **MCP (Model Context Protocol)** based AI analysis feature, allowing natural language conversations with news data for deep analysis.
...
Argus MCP service supports standard Model Context Protocol (MCP), can connect to various AI clients supporting MCP for smart analysis.

### Supported Clients

**Note**:
- Replace `/path/to/Argus` with your actual project path
- Windows paths use double backslashes: `C:\\Users\\YourName\\Argus`
   ...
   ```json
   {
     "mcpServers": {
       "argus": {
         ...
         "description": "Argus News Trending Aggregation Analysis"
       }
     }
   }
   ```
...
Any client supporting Model Context Protocol can connect to Argus:
...
**Notes**:
- Replace `/path/to/Argus` with actual project path
