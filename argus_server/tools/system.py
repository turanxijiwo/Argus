...
class SystemManagementTools:
    ...
    def trigger_crawl(self, platforms: Optional[List[str]] = None, save_to_local: bool = False, include_url: bool = False) -> Dict:
        ...
        """
        try:
            from argus.crawler.fetcher import DataFetcher
            from argus.storage.local import LocalStorageBackend
            from argus.storage.base import convert_crawl_results_to_news_data
            from argus.utils.time import get_configured_time, format_date_folder, format_time_filename
...
"""
    ...
    def check_version(self, proxy_url: Optional[str] = None) -> Dict:
        ...
        同时检查 Argus 和 MCP Server 两个组件的版本更新。
        远程版本 URL 从 config.yaml 获取：
        - version_check_url: Argus 版本
        ...
        try:
            ...
            from argus_server import __version__ as mcp_version
            ...
            argus_url = advanced_config.get(
                "version_check_url",
                "https://raw.githubusercontent.com/sansan0/Argus/refs/heads/master/version"
            )
            mcp_url = advanced_config.get(
                "mcp_version_check_url",
                "https://raw.githubusercontent.com/sansan0/Argus/refs/heads/master/version_mcp"
            )
            ...
            # 检查两个版本
            argus_result = check_single_version(
                "Argus", argus_version, argus_url, proxies, headers
            )
            ...
            # 判断是否有任何更新
            any_update = (
                (argus_result.get("success") and argus_result.get("need_update", False)) or
            ...
            return {
                "success": True,
                "summary": {
                    "description": "版本检查结果（Argus + MCP Server）",
