...
from argus.core.loader import _load_webhook_config, _load_notification_config
from argus.notification.batch import (
    truncate_to_bytes,
    get_batch_header,
    get_max_batch_header_size,
    add_batch_headers,
)
from argus.notification.formatters import strip_markdown
from argus.notification.senders import SMTP_CONFIGS
...
<html><head><meta charset="utf-8"><title>Argus 通知</title>
...
def _send_feishu(webhook_url: str, content: str, title: str) -> Dict:
    """飞书发送（纯文本消息，与 argus send_to_feishu 一致）
...
def _send_email(
    ...
    try:
        ...
        msg["From"] = formataddr(("Argus", from_email))
...
def _send_ntfy(server_url: str, topic: str, content: str, title: str, token: str = "") -> Dict:
    """ntfy 发送（接收已适配的 Markdown，与 argus send_to_ntfy 一致）
    ...
    headers = {
        ...
        "Markdown": "yes",
        "Title": "Argus Notification",  # ASCII，避免 HTTP header 编码问题
        "Priority": "default",
        "Tags": "news",
    }
    if token:
    ...
    def _send_batched_ntfy(
        ...
        for i, (srv, topic) in enumerate(zip(servers, topics)):
            ...
            if not (srv and topic):
                continue
            # ntfy.sh 公共服务器用 2s 间隔（与 argus 一致）
