...
def manual_run():
    ...
    try:
        result = subprocess.run(
            ["python", "-m", "argus"], cwd="/app", capture_output=False, text=True
        )
...
def show_status():
    ...
    if supercronic_is_pid1:
        ...
        print("    💡 建议操作:")
        print("       • 重启容器: docker restart argus")
        print("       • 检查容器日志: docker logs argus")

    # 显示日志检查建议
    print("  📋 运行状态检查:")
    print("    • 查看完整容器日志: docker logs argus")
    print("    • 查看实时日志: docker logs -f argus")
    print("    • 手动执行测试: python manage.py run")
    print("    • 重启容器服务: docker restart argus")
...
def show_logs():
    ...
    try:
        ...
        for log_file in log_files:
            ...
            print("📋 无法找到标准日志文件，建议使用: docker logs argus")
            
    except KeyboardInterrupt:
        print("\n👋 退出日志查看")
    except Exception as e:
        print(f"❌ 查看日志失败: {e}")
        print("💡 建议使用: docker logs argus")
...
def restart_supercronic():
    ...
    try:
        ...
        print("  💡 建议重启容器: docker restart argus")
...
💡 常用操作指南:
  ...
  3. 查看日志: logs
     - 实时监控运行情况
     - 也可使用: docker logs argus

  4. 重启服务: restart
     - 由于 supercronic 是 PID 1，需要重启整个容器
     - 使用: docker restart argus
