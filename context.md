...
- The initial 2026-07-14 audit found 49/54 external API tools returned real data at least once. After retiring permanent-dead `get_crossref_events` and verifying the repaired arXiv path, the current external API surface is 50/53 verified at least once, with `search_github_code`, `search_reddit`, and `search_reliefweb` still blocked for distinct authentication, access, or registration reasons.
