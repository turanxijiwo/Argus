...
from argus.report.helpers import format_rank_display
from argus.utils.time import DEFAULT_TIMEZONE, format_iso_time_friendly, convert_time_for_display
from argus.notification.batch import truncate_at_line_boundary
...
def split_content_into_batches(
    ...
    if format_type in ("wework", "bark"):
        ...
        if update_info:
            base_footer += f"\n> Argus 发现新版本 **{update_info['remote_version']}**，当前 **{update_info['current_version']}**"
    elif format_type == "telegram":
        base_footer = f"\n\n更新时间：{now.strftime('%Y-%m-%d %H:%M:%S')}"
        if update_info:
            base_footer += f"\nArgus 发现新版本 {update_info['remote_version']}，当前 {update_info['current_version']}"
        ...
        if update_info:
            base_footer += f"\n<font color='grey'>Argus 发现新版本 {update_info['remote_version']}，当前 {update_info['current_version']}</font>"
    elif format_type == "dingtalk":
        base_footer = f"\n\n> 更新时间：{now.strftime('%Y-%m-%d %H:%M:%S')}"
        if update_info:
            base_footer += f"\n> Argus 发现新版本 **{update_info['remote_version']}**，当前 **{update_info['current_version']}**"
        ...
        if update_info:
            base_footer += f"\n_Argus 发现新版本 *{update_info['remote_version']}*，当前 *{update_info['current_version']}_"
