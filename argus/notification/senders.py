...
def _feishu_sign(secret: str, timestamp: int) -> str:
    """生成飞书 Webhook 签名校验 (HMAC-SHA256 + base64)"""
...
def _render_ai_analysis(ai_analysis: Any, channel: str) -> str:
    ...
    try:
        from argus.ai.formatter import get_ai_analysis_renderer
...
def send_to_feishu(
    ...
    *,
    ...
    for i, batch_content in enumerate(batches, 1):
        ...
        # 签名校验模式: 在 payload 根追加 timestamp + sign
        if secret:
            ts = int(time.time())
    ...
    *,
    ...
    for i, batch_content in enumerate(batches, 1):
        ...
        payload = {
            "msgtype": "markdown",
            "markdown": {
                "title": f"Argus 热点分析报告 - {report_type}",
                "text": batch_content,
            },
        }
...
def send_to_email(
    ...
    *,
    ...
    try:
        ...
        subject = f"Argus 热点分析报告 - {report_type} - {now.strftime('%m月%d日 %H:%M')}"
