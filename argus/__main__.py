...
from argus.core import load_config, parse_multi_account_config, validate_paired_configs
from argus.core.analyzer import convert_keyword_stats_to_platform_stats
from argus.crawler import DataFetcher
from argus.storage import convert_crawl_results_to_news_data
from argus.utils.time import DEFAULT_TIMEZONE, is_within_days, calculate_days_old
from argus.ai import AIAnalyzer, AIAnalysisResult
from argus.core.scheduler import ResolvedSchedule
...
class NewsAnalyzer:
    ...
    def __init__(self, config: Optional[Dict] = None):
        ...
        if config is None:
            print("正在加载配置...")
            config = load_config()
        print(f"Argus v{__version__} 配置加载完成")
    ...
    def _crawl_rss_data(self) -> Tuple[Optional[List[Dict]], Optional[List[Dict]], Optional[List[Dict]], set]:
        ...
        try:
            from argus.crawler.rss import RSSFetcher, RSSFeedConfig
    ...
    def _process_rss_data_by_mode(self, rss_data) -> Tuple[Optional[List[Dict]], Optional[List[Dict]], Optional[List[Dict]], set]:
        ...
        """
        from argus.core.analyzer import count_rss_frequency
    ...
    def _filter_rss_by_keywords(self, rss_items: List[Dict]) -> List[Dict]:
        ...
        try:
            ...
            if word_groups or filter_words or global_filters:
                from argus.core.frequency import matches_word_groups
                filtered_items = []
                for item in rss_items:
                    title = item.get("title", "")
...
def _run_doctor(config_path: Optional[str] = None) -> bool:
    """运行环境体检"""
    print("=" * 60)
    print(f"Argus v{__version__} 环境体检")
    print("=" * 60)
...
def _build_test_report_data(ctx: AppContext) -> Dict:
    ...
    title = f"Argus 通知测试消息（{now.strftime('%Y-%m-%d %H:%M:%S')}）"
                ...
                "titles": [
                    {
                        "title": title,
                        "source_name": "Argus",
                        "url": "https://github.com/sansan0/Argus",
                        "mobile_url": "",
                        "ranks": [1],
...
<html lang="zh-CN">
<head><meta charset="UTF-8"><title>Argus 通知测试</title></head>
<body>
<h2>Argus 通知连通性测试</h2>
...
def _run_test_notification(config: Dict) -> bool:
    """发送测试通知到已配置渠道"""
    from argus.notification import NotificationDispatcher
...
示例:
  python -m argus                    # 正常运行
  python -m argus --show-schedule    # 查看当前调度状态
  python -m argus --doctor           # 运行一键体检
  python -m argus --test-notification # 测试通知渠道连通性
"""
    )
    parser.add_argument(
        "--show-schedule",
...
def _handle_status_commands(config: Dict) -> None:
    ...
    print("=" * 60)
    print(f"Argus v{__version__} 调度状态")
    print("=" * 60)
